...
d3.csv("updated_combined_data_with_russia.csv").then(data => {
  ...
  const width = 960;
  const height = 600;
  const colorScale = d3.scaleLinear().domain([1, 5]).range(["#f0f9e8", "#08589e"]); // Light to Dark Blue for better contrast
  ...
  d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(geoData => {
    ...
    svg.selectAll("path")
       ...
       .attr("fill", d => {
         ...
         return countryData && countryData.StabilityEstimate ? colorScale(countryData.StabilityEstimate) : "#ccc";
       })
       .attr("stroke", "#fff")
  ...
  function showCountryData(country) {
    ...
    // Separate Plot for Arms Deliveries
    const armsTrace = {
      x: years,
      y: armsDeliveries,
      mode: 'lines+markers',
